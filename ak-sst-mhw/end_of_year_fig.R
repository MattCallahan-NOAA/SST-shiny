...
base_update <- httr::content(httr::GET('https://apex.psmfc.org/akfin/data_marts/akmp/ecosystem_sub_crw_avg_sst?start_date=19850101&end_date=20231130'), type = "application/json") %>% 
  bind_rows %>% 
...
pa2 <- ggplot(data = AIclim_cat %>% filter(t>=as.Date(paste0(last.year,"-12-01"))), aes(x = t, y = temp)) +
                  ...
                  theme(strip.text=element_text(size=20),
                        ...
                        plot.margin=unit(c(-0.7,0.05,3,0),"cm"))
pa2b <- ggdraw(pa2 + 
                   annotate("text",x=0.5,y=0.065,label="NOAA Coral Reef Watch data, courtesy NOAA Pacific Islands Ocean Observing System (Updated: 11/29/2023)                                                              \n Data are modeled satellite products and periodic discrepancies or gaps may exist across sensors and products.\n                                    Contact: Jordan.Watson@noaa.gov, Alaska Fisheries Science Center ",
...
png("AI_112923.png", height=14.25, width=18, units="in", res=300)
print(pa3)
dev.off()
